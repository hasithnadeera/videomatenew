                ...
                {pricingPlans.map((plan, index) => (
                  <div key={plan.name} className="flex-shrink-0 w-64" style={{ scrollSnapAlign: 'center' }}>
                    <div
                      ...
                      className={`relative backdrop-blur-xl rounded-4xl p-4 border overflow-hidden h-96 cursor-pointer transition-all duration-300 hover:scale-105 ${plan.name === 'Pro'
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-8 max-w-5xl mx-auto mb-16">
            {pricingPlans.map((plan, index) => (
              <div
                key={index}
                onClick={handlePricingCardClick}
                className={`relative backdrop-blur-xl rounded-4xl p-4 sm:p-6 md:p-8 border ${plan.isPopular
